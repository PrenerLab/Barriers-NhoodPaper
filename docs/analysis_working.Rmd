...
```{r}
model.poisson <- glm(crime ~ barriers+popE+storef+scale(p_blk16)+scale(p_pov16)+scale(p_pooh16)+scale(p_vac16)+scale(pc70_16), data=one_km, family=poisson())
...
```{r}
model.nb <- glm.nb(crime ~ barriers + popE + storef + scale(p_blk16) + scale(p_pov16) + scale(p_pooh16) + scale(p_vac16) + scale(pc70_16), data=one_km)
...
```{r}
me.fit<-ME(crime ~ popE + barriers + storef + p_blk16 + p_pov16 + p_pooh16 + p_vac16 + pc70_16, data = one_km, family = negative.binomial(1.419), listw = weights, verbose = TRUE, alpha = .05)
```
...
```{r}
model.nb2 <- glm.nb(crime ~ barriers + popE + storef + scale(p_blk16) + scale(p_pov16) + scale(p_pooh16) + scale(p_vac16) + scale(pc70_16) + fitted(me.fit), data = one_km)
...
```
ALgorithm convergence issues—may be ok given the lack of statistically significant Moran's I, but let's run through the motions anyways.
```{r}
b_me.fit
```
...
```{r}
b_model.zoin <- zeroinfl(barriers ~ popE + scale(p_blk16) + scale(p_pooh16) + scale(p_vac16) + scale(p_pov16) + scale(pc70_16), dist = "poisson", data = one_km)
...
```{r}
b_model2.zoin <- zeroinfl(barriers ~ popE + p_blk16 + p_pooh16 + p_vac16 + p_pov16 + fitted(b_me.fit), dist = "poisson", data = one_km)
...
```{r}
b80_model.nb2 <- glm.nb(barriers ~ pop80 + scale(p_blk80) + scale(p_vac80) + scale(p_pov80) + scale(pc50_80) + fitted(b80_me.fit), data = one_km)
...
Zero-inflated model
```{r}
b80_model.zoin <- zeroinfl(barriers ~ pop80 + scale(p_blk80) + scale(p_vac80) + scale(p_pov80) + scale(pc50_80), dist = "poisson", data = one_km)
...
Zero-inflation barriers model with eigenvector filtering
```{r}
b80_model2.zoin <- zeroinfl(barriers ~ pop80 + scale(p_blk80) + scale(p_vac80) + scale(p_pov80) + 
